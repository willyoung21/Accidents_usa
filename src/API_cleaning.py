...
def clean_api_data():
    ...
    # Convert `crash_date` and `crash_time` to datetime format
    ...
    # Fix inconsistent values (e.g., remove whitespace or correct capitalization in the `borough` column)
    ...
    # Remove duplicates based on the `collision_id` column (assuming it's unique for each accident)
